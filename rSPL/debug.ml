...
struct
  ...
  let string_init n f = 
    let rec visit n acc = 
      if n = 0 then acc else visit (n-1) (acc^(f n))
    in visit n ""
  ...
  let debug_calls  =
    ...
    object (self)
      ...
      method print_call s call_site =
        let spaces n = string_init n (fun _ -> " ") in (* return a string of length n with n spaces*)
...
let parse option_flag = 
  ...
  let _ = Arg.parse option_flag (fun s -> VarGen.file := s) VarGen.usage in
  let _ = read_main () in
  ()
